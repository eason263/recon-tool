...
def test_batch_load_manifest(client, tmp_path):
    m = tmp_path / "manifest.csv"
    m.write_text("source,target\n/a.csv,/b.csv\n/c.xlsx,/d.xlsx\n")
    resp = client.post("/batch/load-manifest", data={"manifest": (m.open("rb"), "manifest.csv")},
                       content_type="multipart/form-data")
    assert resp.status_code == 200
    data = resp.get_json()
    assert data["pairs"] == [{"source": "/a.csv", "target": "/b.csv"},
                              {"source": "/c.xlsx", "target": "/d.xlsx"}]
